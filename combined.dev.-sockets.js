  ...
  function b(q, g) {
    ...
    this.update = function (z) {
      ...
      d.ratio =
        g.sizethumb === "auto"
          ? h[g.axis] / m[g.axis]
          : (h[g.axis] - j[g.axis]) / (m[g.axis] - p[g.axis]);
      r =
        z === "relative" && h.ratio <= 1
          ? Math.min(h[g.axis] - j[g.axis], Math.max(0, r))
          : 0;
      r =
        z === "bottom" && h.ratio <= 1
          ? h[g.axis] - j[g.axis]
    ...
    function u(z) {
      if (h.ratio < 1) {
        if (g.invertscroll && e) {
          y.now = Math.min(
            m[g.axis] - p[g.axis],
            Math.max(0, y.start + (o.start - (l ? z.pageX : z.pageY)))
          );
        } else {
          y.now = Math.min(
            m[g.axis] - p[g.axis],
            Math.max(0, y.start + ((l ? z.pageX : z.pageY) - o.start))
          );
        }
        r = y.now * d.ratio;
        h.obj.css(n, -r);
