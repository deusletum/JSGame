...
function iframeLoaded(requestUrl, externalOptions){
		...
		try {
			if(window.ActiveXObject) {
				...
				for(var i in navigator.plugins) {
					...
					if (plugin.name && plugin.name.match(/flash/i)) {
	...
	try {
		...
		if (externalOptions._direction == 'game') {
			var img = new Image();
			var url = requestUrl + 'iframeLoaded.php?';
			img.src = url + parameters.join('&');
		} else {
			options.screenWidth = window.screen.availWidth;
			options.screenHight = window.screen.availHeight;
			options.query = encodeURIComponent(document.location.search);
