...
Feedback.Controller = function(options){
	...
	$('.button_show_feedback_form').on('click', function() {
		NXG.SendButtonClicks('support');
		self.showForm();
	});
	...
	this.$form.on('mouseup', function(e){
		...
		if ($selectbox.length > 0) {
			...
			$selectbox.find('.form-feedback-selectbox-value')
				.text( $self.text() )
				.attr('data-value', $self.attr('data-value') );
...
Feedback.Controller.prototype.testNetworks = function(urls){
		...
		(function measure(servers){
			...
			if (server) {
				...
				var callbackServer = function(event){
					...
					ts = (event && event.type && event.type === 'load') ? ((currentTS - ts) / 1000) : '-';
				...
				if (server.type === 'xml') {
					...
					script.onload = script.onerror = script.onabort = callbackServer;
					timeoutId = setTimeout(callbackServer, 5000);
					document.body.appendChild(script);
				} else {
					var image = new Image();
					image.onload = image.onerror = image.onabort = callbackServer;
	...
	return promise(function() {
		if (nxg.getModule('pushd')._status === 'connected') {
...
Feedback.Controller.prototype.showForm = function(){
	...
	var game = this.$form.find('input[name="fields[21727158]"]').val();
	$li = this.$form.find('.form-feedback-selectbox li');

	$li.each(function(){

		// data-enabled-in="*,-_fm"
		var enabledInString = $(this).data('enabled-in');
		var enabledIn = enabledInString ? enabledInString.split(',') : ['*'];

		var showItem = (enabledIn.indexOf('*') || enabledIn.indexOf(game)) > enabledIn.indexOf('-' + game);

		$(this).toggleClass('disable', showItem);
	});
